  ...
  if (atLimit) {
    return (
      <p className="mt-3 text-center text-[11.5px] text-[#9ea8c4]">
        Conversation limit reached. Regenerate to start fresh.
      </p>
    );
  }
...
export default function StrategyPanel({
  ...
  // Per-tab follow-up chat: history + streaming tail
  const [chatHistory, setChatHistory] = useState<
    Partial<Record<StrategyAction, ChatMessage[]>>
  >({});
  const [chatStreaming, setChatStreaming] = useState<
    Partial<Record<StrategyAction, string>>
  >({});
  ...
  function handleRegenerate() {
    ...
    // Clear chat history for this tab so it doesn't reference a stale response
    setChatHistory((prev) => {
      const next = { ...prev };
      delete next[active];
      return next;
  ...
  const sendChatMessage = useCallback(
    async (userText: string) => {
      if (!active || phase !== "done" || !summary) return;

      const actionHistory = chatHistory[active] ?? [];

      // Optimistically append user message
      const userMsg: ChatMessage = { role: "user", content: userText };
      const newHistory = [...actionHistory, userMsg];
      setChatHistory((prev) => ({ ...prev, [active]: newHistory }));
      setChatBusy(true);
      setChatStreaming((prev) => ({ ...prev, [active]: "" }));

      // The "original response" is the prose summary + strategy title as plain text
      const originalText = strategy
        ? `${summary}\n\n${strategy.title}`
        : summary;
      ...
      try {
        ...
        if (!res.ok || !res.body) {
          const err = res.ok ? "No response body" : ((await res.json()) as { error?: string }).error ?? "Chat failed";
          setChatHistory((prev) => ({
            ...prev,
            [active]: [
              ...(prev[active] ?? []),
          ...
          for (const part of parts) {
            ...
            try {
              ...
              if (event.type === "chunk" && event.text) {
                ...
                setChatStreaming((prev) => ({ ...prev, [active]: accumulated }));
                ...
                setChatHistory((prev) => ({
                  ...
                  [active]: [
                    ...(prev[active] ?? []),
                    { role: "assistant", content: `Error: ${event.message ?? "Something went wrong"}` },
                  ],
                }));
                setChatStreaming((prev) => ({ ...prev, [active]: "" }));
              }
    ...
    [active, phase, summary, strategy, date, chatHistory],
  );
            ...
            {active && (
              <>
                <ChatThread
                  ...
                  streamingText={chatStreaming[active] ?? ""}
                />
                <ChatInput
                  onSend={sendChatMessage}
                  disabled={chatBusy || isWorking}
                  atLimit={(chatHistory[active]?.filter((m) => m.role === "user").length ?? 0) >= MAX_CHAT_TURNS}
                />
              </>
            )}
