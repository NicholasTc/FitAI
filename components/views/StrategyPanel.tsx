    ...
    async (userText: string) => {
      ...
      try {
        await streamChatResponse(
          { date, action: active, originalResponse: originalText, history: newHistory },
          (accumulated) => setChatStreaming((prev) => ({ ...prev, [active]: accumulated })),
          (full) => {
            setChatHistory((prev) => ({
              ...prev,
              [active]: [...(prev[active] ?? []), { role: "assistant", content: full }],
            }));
            setChatStreaming((prev) => ({ ...prev, [active]: "" }));
          },
          (msg) => {
            setChatHistory((prev) => ({
              ...prev,
              [active]: [...(prev[active] ?? []), { role: "assistant", content: `Error: ${msg}` }],
            }));
            setChatStreaming((prev) => ({ ...prev, [active]: "" }));
          },
        );
